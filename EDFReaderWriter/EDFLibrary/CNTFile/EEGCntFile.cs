...
namespace CNTFile
{
    ...
    public class EEGCntFile
    {
        ...
        //unsure what this does, maybe just some predefined channel names?
        ...
        //empty constructor, have to use ReadFile to do anything useful
        public EEGCntFile()
        {
        }
        ...
        //eeg data is stored as eeg_data[channel, sample]
        ...
        //cntf -> cnt file name | freader is a binary reader to read bytes from the stream
        public bool ReadCnt(string cntf)
        {
            ...
            // read header file | int32  = (2 * size of one of the data records in EDF)
            ...
            if (amp_info.data_size > 4) 
                amp_info.data_size = 4;
            ...
            // ccwang  - version 1.1 -> not sure what all this is about
            ...
            //nspl = number of samples, ispl = current sample index
            for (int ispl = 0; ispl < nspl; ispl++)
            {
                // read eeg -> so data is organised -> (chan1.data[0] chan2.data[0]..chann.data[0] and THEN goes to next data line)
                ...
                //read events -> 0xff is a mask that only returns last byte out of the 4 byte integer
            ...
            if (amp_info.chan_names == null && amp_info.num_chan == 40)
            { //NUAmps
